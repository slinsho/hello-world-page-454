...
import { Upload, X, Camera, Building2, User, ArrowLeft, ShieldCheck } from "lucide-react";
import { z } from "zod";
...
const Verification = () => {
  ...
  const openCamera = async () => { try { const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "user", width: { ideal: 1280 }, height: { ideal: 720 } }, audio: false }); streamRef.current = stream; setIsCameraOpen(true); } catch (error) { console.error("Camera error:", error); toast({ title: "Camera Error", description: "Unable to access camera.", variant: "destructive" }); } };
  useEffect(() => { if (isCameraOpen && streamRef.current && videoRef.current) { videoRef.current.srcObject = streamRef.current; videoRef.current.play().catch(err => console.error("Error playing video:", err)); } }, [isCameraOpen]);
  const capturePhoto = () => { if (!videoRef.current) return; const canvas = document.createElement("canvas"); canvas.width = videoRef.current.videoWidth; canvas.height = videoRef.current.videoHeight; const ctx = canvas.getContext("2d"); if (!ctx) return; ctx.drawImage(videoRef.current, 0, 0); canvas.toBlob((blob) => { if (blob) { setSelfieImage(new File([blob], `selfie-${Date.now()}.jpg`, { type: "image/jpeg" })); closeCamera(); } }, "image/jpeg", 0.95); };
  const closeCamera = () => { if (streamRef.current) { streamRef.current.getTracks().forEach(track => track.stop()); streamRef.current = null; } setIsCameraOpen(false); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const idImage of idImages) { const fileExt = idImage.name.split(".").pop(); const fileName = `${user.id}/id-${Date.now()}-${Math.random()}.${fileExt}`; const { error: uploadError } = await supabase.storage.from("verification-docs").upload(fileName, idImage); if (uploadError) throw uploadError; idPaths.push(fileName); }
        ...
        <form onSubmit={handleSubmit} className="space-y-5 md:grid md:grid-cols-2 md:gap-6 md:space-y-0">
          ...
          <div className="space-y-5">
            {isAgent && (
              <div className="bg-card rounded-2xl p-4 border border-border space-y-3">
                ...
                <div className="space-y-1.5"><Label htmlFor="businessPhone" className="text-xs text-muted-foreground">Business Phone *</Label><Input id="businessPhone" type="tel" value={formData.businessPhone} onChange={(e) => setFormData({ ...formData, businessPhone: e.target.value })} placeholder="+231..." required className="rounded-xl" /></div>
              </div>
            )}
            <div className="bg-card rounded-2xl p-4 border border-border space-y-4">
              ...
              <div className="space-y-1.5"><Label htmlFor="dob" className="text-xs text-muted-foreground">Date of Birth *</Label><Input id="dob" type="date" value={formData.dateOfBirth} onChange={(e) => setFormData({ ...formData, dateOfBirth: e.target.value })} required className="rounded-xl" /></div>
              <div className="space-y-1.5"><Label className="text-xs text-muted-foreground">National ID Type *</Label><Select value={formData.idType} onValueChange={(value: any) => setFormData({ ...formData, idType: value })}><SelectTrigger className="rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="citizen_card">Citizen Card</SelectItem><SelectItem value="voter_card">Voter Card</SelectItem><SelectItem value="passport">Passport</SelectItem></SelectContent></Select></div>
            </div>
            <div className="bg-card rounded-2xl p-4 border border-border space-y-3">
          ...
          <div className="space-y-5">
            <div className="bg-card rounded-2xl p-4 border border-border space-y-3">
              ...
              {selfieImage && !isCameraOpen && (<div className="relative w-40 mx-auto"><img src={URL.createObjectURL(selfieImage)} alt="Selfie" className="w-full h-40 object-cover rounded-2xl" /><Button type="button" variant="destructive" size="icon" className="absolute top-1.5 right-1.5 h-6 w-6 rounded-full" onClick={() => setSelfieImage(null)}><X className="h-3.5 w-3.5" /></Button></div>)}
            </div>
            ...
            {isAgent && (
              <div className="bg-card rounded-2xl p-4 border border-border space-y-4">
                ...
                <div className="space-y-1.5"><Label htmlFor="agencyName" className="text-xs text-muted-foreground">Agency Name *</Label><Input id="agencyName" value={formData.agencyName} onChange={(e) => setFormData({ ...formData, agencyName: e.target.value })} placeholder="Your agency or company name" required className="rounded-xl" /></div>
                <div className="space-y-1.5"><Label htmlFor="officeLocation" className="text-xs text-muted-foreground">Office Location *</Label><Input id="officeLocation" value={formData.officeLocation} onChange={(e) => setFormData({ ...formData, officeLocation: e.target.value })} placeholder="e.g., Congo Town, Monrovia" required className="rounded-xl" /></div>
