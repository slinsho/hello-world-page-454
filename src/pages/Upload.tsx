...
const Upload = () => {
  ...
  useEffect(() => {
    ...
    const checkVerificationAndRole = async () => {
      const { data } = await supabase.from("profiles").select("verification_status, role, phone, contact_phone_2").eq("id", user.id).single();
      if (data) {
        setVerificationStatus(data.verification_status);
        setUserRole(data.role);
        if (data.phone) setFormData(prev => ({ ...prev, contact_phone: data.phone || "" }));
        if (data.contact_phone_2) setFormData(prev => ({ ...prev, contact_phone_2: data.contact_phone_2 || "" }));
      }
    ...
    return (
      <div className="min-h-screen bg-background"><Navbar />
        <div className="px-4 pt-16 text-center max-w-sm mx-auto md:max-w-lg">
          <div className="h-20 w-20 rounded-full bg-blue-500/10 flex items-center justify-center mx-auto mb-6"><Building2 className="h-10 w-10 text-blue-500" /></div>
          <h2 className="text-xl font-bold mb-2">Property Limit Reached</h2>
          <p className="text-sm text-muted-foreground mb-4">Owner accounts can list up to 2 properties. To upload more, upgrade to an Agent account.</p>
          <p className="text-xs text-muted-foreground mb-6">You can also delete an existing property to make room for a new one.</p>
          <div className="space-y-3"><Button onClick={() => navigate("/verification?upgrade=agent")} className="w-full rounded-full">Upgrade to Agent</Button><Button variant="outline" onClick={() => navigate("/profile")} className="w-full rounded-full">Manage My Properties</Button></div>
        </div>
      </div>
    );
  }

  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => { const files = Array.from(e.target.files || []); setPhotos((prev) => [...prev, ...files].slice(0, 10)); };
  ...
  const handleVideoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validFormats.includes(file.type)) { toast({ title: "Invalid Format", description: "Only MP4 and MOV formats allowed.", variant: "destructive" }); e.target.value = ''; return; }
    const maxSize = 10 * 1024 * 1024;
    if (file.size > maxSize) { toast({ title: "File Too Large", description: "Video must be 10MB or less.", variant: "destructive" }); e.target.value = ''; return; }
    const video = document.createElement('video');
    video.preload = 'metadata';
    video.onloadedmetadata = function() { window.URL.revokeObjectURL(video.src); if (video.duration > 20) { toast({ title: "Video Too Long", description: "Video must be 20 seconds or less.", variant: "destructive" }); e.target.value = ''; return; } setVideos([file]); };
    video.onerror = function() { toast({ title: "Error", description: "Failed to load video file.", variant: "destructive" }); e.target.value = ''; };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else { toast({ title: "Error", description: error.message || "Failed to upload property", variant: "destructive" }); }
  ...
  if (verificationStatus !== "approved") {
    return (
      <div className="min-h-screen bg-background"><Navbar />
        <div className="px-4 pt-16 text-center max-w-sm mx-auto md:max-w-lg">
          <div className="h-20 w-20 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-6"><FileText className="h-10 w-10 text-primary" /></div>
          <h2 className="text-xl font-bold mb-2">Verification Required</h2>
          <p className="text-sm text-muted-foreground mb-6">You need to be verified before you can upload properties.</p>
          <Button onClick={() => navigate("/profile")} className="rounded-full px-8">Go to Profile</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background pb-24 md:pb-8">
      ...
      <main className="max-w-lg mx-auto px-4 pt-4 md:max-w-5xl md:px-6 md:pt-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div><h1 className="text-xl md:text-2xl font-bold">List Property</h1><p className="text-xs text-muted-foreground">Add your property details</p></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="md:grid md:grid-cols-2 md:gap-8 space-y-5 md:space-y-0">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Input type="tel" value={formData.contact_phone} onChange={(e) => !isOwner && setFormData({ ...formData, contact_phone: e.target.value })} required maxLength={20} placeholder="Phone 1 *" className="rounded-xl h-12" readOnly={isOwner} disabled={isOwner} />
          ...
          <div className="space-y-5">
            <div className="space-y-3">
              <Label className="text-sm font-semibold flex items-center gap-2"><Camera className="h-4 w-4 text-muted-foreground" />Photos <span className="text-muted-foreground font-normal">({photos.length}/10)</span></Label>
              <div className="border-2 border-dashed border-border rounded-2xl p-6 text-center bg-card">
                <input type="file" accept="image/*" multiple onChange={handlePhotoChange} className="hidden" id="photo-upload" disabled={photos.length >= 10} />
                ...
                <div className="grid grid-cols-4 gap-2">
                  {photos.map((photo, index) => (
                    <div key={index} className="relative aspect-square rounded-xl overflow-hidden">
                      ...
                      <button type="button" className="absolute top-1 right-1 h-6 w-6 rounded-full bg-destructive flex items-center justify-center" onClick={() => removePhoto(index)}><X className="h-3 w-3 text-destructive-foreground" /></button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-3">
              ...
              <div className="border-2 border-dashed border-border rounded-2xl p-6 text-center bg-card">
                ...
                <label htmlFor="video-upload" className="cursor-pointer flex flex-col items-center gap-2"><div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center"><Video className="h-6 w-6 text-primary" /></div><span className="text-sm text-muted-foreground">Tap to add video</span></label>
              </div>
              {videos.length > 0 && (
                <div className="relative rounded-2xl overflow-hidden">
                  <video src={URL.createObjectURL(videos[0])} className="w-full h-48 object-cover rounded-2xl" controls />
                  <button type="button" className="absolute top-2 right-2 h-7 w-7 rounded-full bg-destructive flex items-center justify-center" onClick={() => removeVideo(0)}><X className="h-4 w-4 text-destructive-foreground" /></button>
                </div>
              )}
            </div>
            ...
            <div className="bg-card rounded-2xl p-4 border border-border">
              <div className="flex items-start gap-3">
                <Checkbox id="agreement" checked={agreed} onCheckedChange={(checked) => setAgreed(checked === true)} className="mt-0.5" />
                <label htmlFor="agreement" className="text-sm text-muted-foreground leading-relaxed cursor-pointer">
                  ...
                  <span className="text-primary cursor-pointer" onClick={() => navigate("/terms")}>Terms & Conditions</span>.
                </label>
              </div>
            </div>

            <Button type="submit" className="w-full h-14 rounded-2xl text-base font-semibold" disabled={loading || !agreed}>
