      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Your property analytics</p>
          </div>
          <Button 
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
            ...
            <div className="flex items-start justify-between mb-2">
              ...
              {totalStats.viewsTrend !== 0 && (
                <div className={`flex items-center gap-0.5 text-xs font-medium ${totalStats.viewsTrend > 0 ? "text-green-500" : "text-red-500"}`}>
                  {totalStats.viewsTrend > 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                  {Math.abs(totalStats.viewsTrend)}%
                </div>
              )}
            </div>
          ...
          <div className="bg-card rounded-2xl p-4 border border-border">
            ...
            <p className="text-2xl font-bold">{totalStats.totalInquiries.toLocaleString()}</p>
        ...
        <div className="bg-card rounded-2xl p-4 border border-border mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="font-semibold">Views Trend</h2>
          ...
          {properties.length === 0 ? (
            ...
            <div className="space-y-3">
              {properties.slice(0, 5).map((prop) => (
                <div
                  key={prop.id}
                  className="bg-card rounded-2xl p-3 border border-border flex items-center gap-3 cursor-pointer hover:bg-muted/50 active:scale-[0.99] transition-all"
