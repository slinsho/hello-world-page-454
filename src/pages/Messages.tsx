...
export default function Messages() {
  ...
  useEffect(() => {
    if (user && !showUpgrade) fetchConversations();
  }, [user, showUpgrade]);
  ...
  const fetchConversations = async () => {
    ...
    const { data, error } = await supabase
      .from("conversations")
      .select(`*, properties:property_id (title, photos)`)
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("messages").insert({ conversation_id: activeConversation, sender_id: user.id, content: newMessage.trim() });
  ...
  if (showUpgrade) {
    return (
      <div className="min-h-screen bg-background">
        <Navbar />
        <UpgradeToAgentDialog open={true} onOpenChange={(open) => { if (!open) navigate(-1); }} featureName="Messages" />
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background flex flex-col">
        <div className="sticky top-0 z-50 bg-background/95 backdrop-blur-lg border-b border-border">
          <div className="flex items-center gap-3 p-3">
            <Button variant="ghost" size="icon" className="shrink-0 h-10 w-10 rounded-full" onClick={() => setActiveConversation(null)}>
            ...
            <div className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer" onClick={() => activeConvo.other_user && navigate(`/profile/${activeConvo.other_user.id}`)}>
              <div className="relative">
                <Avatar className="h-11 w-11 ring-2 ring-primary/20">
                  ...
                  <AvatarFallback className="bg-primary/10 text-primary font-semibold">{activeConvo.other_user?.name?.charAt(0) || "U"}</AvatarFallback>
                </Avatar>
                <div className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-green-500 border-2 border-background" />
              </div>
              <div className="flex-1 min-w-0">
                <h2 className="font-semibold text-base truncate">{activeConvo.other_user?.name || "Unknown"}</h2>
          ...
          {messageGroups.map((group) => (
            <div key={group.date}>
              <div className="flex items-center justify-center my-4">
                <span className="px-3 py-1 text-xs font-medium text-muted-foreground bg-muted rounded-full">{getDateLabel(group.date)}</span>
              </div>
                    ...
                    <div key={msg.id} className={`flex items-end gap-2 ${isOwn ? "justify-end" : "justify-start"}`}>
                        ...
                        <div className="w-8">
                          {showAvatar && (
                            <Avatar className="h-8 w-8">
                              <AvatarImage src={activeConvo.other_user?.profile_photo_url || undefined} />
                              <AvatarFallback className="text-xs bg-muted">{activeConvo.other_user?.name?.charAt(0) || "U"}</AvatarFallback>
                            </Avatar>
                          )}
                        </div>
                      )}
                      <div className={`max-w-[75%] px-4 py-2.5 ${isOwn ? `bg-primary text-primary-foreground ${isLast ? "rounded-2xl rounded-br-md" : "rounded-2xl"}` : `bg-muted ${isLast ? "rounded-2xl rounded-bl-md" : "rounded-2xl"}`}`}>
                        ...
                        <div className={`flex items-center justify-end gap-1 mt-1 ${isOwn ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
                          <span className="text-[10px]">{formatMessageTime(new Date(msg.created_at))}</span>
        ...
        <div className="sticky bottom-0 bg-background/95 backdrop-blur-lg border-t border-border p-3 pb-6 md:pb-3">
          <form onSubmit={sendMessage} className="flex items-center gap-2">
            <div className="flex-1 relative">
              <Input ref={inputRef} value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type a message..." disabled={sendingMessage} className="rounded-full pl-4 pr-4 py-6 bg-muted border-0 focus-visible:ring-1 focus-visible:ring-primary" />
            </div>
            <Button type="submit" size="icon" disabled={sendingMessage || !newMessage.trim()} className="h-12 w-12 rounded-full shrink-0">
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <main className="max-w-2xl mx-auto">
        <div className="sticky top-0 z-40 bg-background/95 backdrop-blur-lg px-4 pt-4 pb-2">
        ...
        <div className="divide-y divide-border">
          {loading ? (
            ...
            filteredConversations.map((convo) => (
              <div
                key={convo.id}
                className={`flex items-center gap-3 p-4 cursor-pointer hover:bg-muted/50 active:bg-muted transition-colors ${convo.unread_count && convo.unread_count > 0 ? "bg-primary/5" : ""}`}
                onClick={() => setActiveConversation(convo.id)}
              >
                <div className="relative">
                  <Avatar className="h-14 w-14">
                    <AvatarImage src={convo.other_user?.profile_photo_url || undefined} />
                    <AvatarFallback className="bg-primary/10 text-primary font-semibold text-lg">{convo.other_user?.name?.charAt(0) || "U"}</AvatarFallback>
                  </Avatar>
                  <div className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-green-500 border-2 border-background" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h3 className={`font-semibold truncate ${convo.unread_count && convo.unread_count > 0 ? "text-foreground" : "text-foreground"}`}>{convo.other_user?.name || "Unknown"}</h3>
                    <span className="text-xs text-muted-foreground shrink-0 ml-2">{formatMessageDate(new Date(convo.last_message_at))}</span>
                  </div>
                  <div className="flex items-center justify-between mt-0.5">
                    ...
                    {convo.unread_count && convo.unread_count > 0 ? (
                      <Badge className="ml-2 h-5 min-w-[20px] flex items-center justify-center rounded-full text-[10px] px-1.5">{convo.unread_count}</Badge>
                    ) : null}
                  </div>
