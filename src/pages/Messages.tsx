        ...
        <div className="sticky top-0 z-50 bg-background/95 backdrop-blur-lg border-b border-border">
          <div className="flex items-center gap-3 p-3">
            ...
            <div 
              ...
              onClick={() => activeConvo.other_user && navigate(`/profile/${activeConvo.other_user.id}`)}
            >
            ...
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full hidden sm:flex">
          ...
          {activeConvo.property && (
            <div 
              ...
              onClick={() => navigate(`/property/${activeConvo.property_id}`)}
            >
              {activeConvo.property.photos?.[0] && (
                <img 
                  src={activeConvo.property.photos[0]} 
                  alt="" 
                  className="h-8 w-8 rounded-md object-cover"
                />
              )}
              <p className="text-xs text-muted-foreground truncate flex-1">
                Discussing: <span className="text-foreground font-medium">{activeConvo.property.title}</span>
              </p>
            </div>
          )}
        </div>
                      ...
                      <div
                        className={`max-w-[75%] px-4 py-2.5 ${
                          isOwn
                            ...
                            : `bg-muted ${isLast ? "rounded-2xl rounded-bl-md" : "rounded-2xl"}`
                        }`}
                      >
          ...
          <form onSubmit={sendMessage} className="flex items-center gap-2">
            <div className="flex-1 relative">
              <Input
                ...
                className="rounded-full pl-4 pr-4 py-6 bg-muted border-0 focus-visible:ring-1 focus-visible:ring-primary"
              />
            </div>
            <Button 
              type="submit" 
        ...
        <div className="divide-y divide-border">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center px-4">
              ...
              <Button className="mt-4 rounded-full" onClick={() => navigate("/explore")}>
                Browse Properties
              </Button>
            </div>
          ) : (
            filteredConversations.map((convo) => (
              <div
                key={convo.id}
                className="flex items-center gap-3 p-4 cursor-pointer hover:bg-muted/50 active:bg-muted transition-colors"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <h3 className={`font-semibold truncate ${(convo.unread_count ?? 0) > 0 ? "text-foreground" : "text-foreground"}`}>
                      {convo.other_user?.name || "Unknown"}
                    </h3>
                    <span className={`text-xs shrink-0 ml-2 ${(convo.unread_count ?? 0) > 0 ? "text-primary font-medium" : "text-muted-foreground"}`}>
                      {formatMessageDate(new Date(convo.last_message_at))}
                    </span>
                  </div>
                  <p className={`text-sm truncate ${(convo.unread_count ?? 0) > 0 ? "text-foreground font-medium" : "text-muted-foreground"}`}>
                    {convo.last_message || (convo.property ? `Re: ${convo.property.title}` : "No messages yet")}
                  </p>
                </div>
              </div>
            ))
